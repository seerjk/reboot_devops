...
def get(**kwargs):
    ...
    order_by_list = check_order_by(Cabinet, order_by)
    ...
    data = db.session.query(Cabinet).filter_by(**where)\
        .order_by(getattr(getattr(Cabinet, order_by_list[0]), order_by_list[1])())\
        .limit(limit).all()
    db.session.close()
