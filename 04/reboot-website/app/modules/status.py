...
def get(**kwargs):
    ...
    order_by_list = check_order_by(Status, order_by)
    ...
    data = db.session.query(Status).filter_by(**where)\
        .order_by(getattr(getattr(Status, order_by_list[0]), order_by_list[1])())\
        .limit(limit).all()
    db.session.close()
