...
def get(**kwargs):
    ...
    # 2 验证
    # 验证output
    check_output_field(ManagementCard, output)

    # 验证 order_by，字符串分割，字段是否在表中   第二个字段必须为asc desc
    order_by_list = check_order_by(ManagementCard, order_by)
    ...
    data = db.session.query(ManagementCard).filter_by(**where)\
        .order_by(getattr(getattr(ManagementCard, order_by_list[0]), order_by_list[1])())\
        .limit(limit).all()
    db.session.close()
